...
#define ET7304M_DRV_VERSION    "2.0.5_MTK"
...
static int et7304_init_vbus_cal(struct tcpc_device *tcpc)
{
    ...
    ret = et7304_block_write(chip->client, ET7304M_REG_UNLOCK_PW_2,
            ARRAY_SIZE(val_en_test_mode), val_en_test_mode);
    if (ret < 0)
        dev_notice(chip->dev, "%s en test mode fail(%d)\n",
                __func__, ret);

    ret = et7304_reg_read(chip->client, ET7304M_REG_EFUSE5);
    if (ret < 0)
        goto out;

    data = ret;
    data = (data & ET7304M_REG_M_VBUS_CAL) >> ET7304M_REG_S_VBUS_CAL;
    cal = (data & BIT(2)) ? (data | GENMASK(7, 3)) : data;
    cal -= 2;
    if (cal < ET7304M_REG_MIN_VBUS_CAL)
        cal = ET7304M_REG_MIN_VBUS_CAL;
    data = (cal << ET7304M_REG_S_VBUS_CAL) | (ret & GENMASK(4, 0));

    ret = et7304_reg_write(chip->client, ET7304M_REG_EFUSE5, data);
out:
    ret = et7304_block_write(chip->client, ET7304M_REG_UNLOCK_PW_2,
